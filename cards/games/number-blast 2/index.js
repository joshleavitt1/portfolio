...
(function () {
      ...
      return new Promise((resolve) => {
        ...
        function getAdjustedAnchor(clientX, clientY, piece, grabCell) {
          const hoverIndex = cellIndexFromPointer(
            clientX,
            clientY - Math.min(window.innerHeight * 0.035, 24)
          );
  ...
  : { x: 0, y: 0 };
              ...
              const onUp = (evUp) => {
                ...
                ghost.style.transition = "transform 110ms cubic-bezier(.22,1,.36,1), opacity 110ms ease";
        ...
        async function placePiece(anchorIndex, handIndex) {
          ...
          // redraw board only first so the placed tiles can visibly "lock in"
          updateTopbar();
          renderGrid(true);
